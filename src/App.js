        ...
        >
          <div className="card-artwork-container">
            <img src={song.albumArt} alt={song.title} className="song-album-art" 
              onError={(e) => { e.target.onerror = null; e.target.src='https://placehold.co/180x180/000000/FFFFFF?text=Error'; }}
            />
                ...
                {isAuthenticated ? (
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="form-group">
                            ...
                            <input type="password" id="password" required className="form-input" placeholder="••••••••" />
                        </div>
                        {!isLoginView && (
                            <div className="form-group">
                                <label htmlFor="confirm-password">Confirm Password</label>
                                <input type="password" id="confirm-password" required className="form-input" placeholder="••••••••" />
                            </div>
                        )}
...
function App() {
  ...
  // --- API & DATA ---
  const JAMENDO_CLIENT_ID = '2838a1f3'; // Public client_id for demonstration
  ...
  useEffect(() => {
    const fetchSongs = async () => {
      ...
      try {
        ...
        if (data.results.length === 0) throw new Error('No songs found. The API might be temporarily unavailable.');
  ...
  return (
    <>
      <style>{`
        ...
        [data-theme='dark'] {
          --bg-primary: #121212; --bg-secondary: #1e1e1e; --bg-tertiary: #2a2a2a; --bg-modal: #282828;
          --text-primary: #ffffff; --text-secondary: #b3b3b3; --border-color: #333333;
          --accent-color: #1DB954; --accent-color-light: rgba(29, 185, 84, 0.2);
          --hover-bg: #282828; --shadow-color: rgba(0, 0, 0, 0.5);
        }
        [data-theme='light'] {
          --bg-primary: #f5f5f7; --bg-secondary: #ffffff; --bg-tertiary: #e5e5e5; --bg-modal: #ffffff;
          --text-primary: #1d1d1f; --text-secondary: #515154; --border-color: #d2d2d7;
          --accent-color: #007aff; --accent-color-light: rgba(0, 122, 255, 0.1);
        ...
        .sidebar-nav { flex-grow: 1; } /* Allows actions to be pushed down */
        ...
        .sidebar-actions { display: flex; gap: 0.5rem; padding-top: 1rem; } /* No margin-top needed */
        ...
        .song-card { width: 180px; flex-shrink: 0; cursor: pointer; transition: transform 0.2s; }
        .song-card:hover { transform: translateY(-4px); }
        .card-artwork-container { position: relative; margin-bottom: 0.75rem; }
        .song-album-art { width: 100%; height: 180px; border-radius: 8px; object-fit: cover; display: block; box-shadow: 0 4px 12px var(--shadow-color); }
        ...
        .song-card:hover .card-play-button, .song-card.active .card-play-button { opacity: 1; transform: translateY(0); }
        .song-card.active .card-play-button { color: var(--accent-color); }
        ...
        .song-artist { font-size: 0.875rem; color: var(--text-secondary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .sound-wave-overlay { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: var(--accent-color-light); border-radius: 8px; display: flex; align-items: center; justify-content: center; color: var(--accent-color); pointer-events: none; }
        ...
        .footer-song-title { font-weight: 600; }
        .footer-song-artist { font-size: 0.875rem; color: var(--text-secondary); }
        .footer-controls { display: flex; flex-direction: column; align-items: center; gap: 0.5rem; }
        .controls-container { display: flex; align-items: center; gap: 1rem; }
        .control-button, .play-pause-button { background: none; border: none; color: var(--text-secondary); cursor: pointer; transition: color 0.2s; }
        ...
        .play-pause-button:hover { transform: scale(1.05); }
        .progress-container { display: flex; align-items: center; gap: 0.75rem; width: 100%; max-width: 500px; }
        ...
        .modal-content { background: var(--bg-modal); padding: 2rem; border-radius: 12px; width: 90%; max-width: 400px; position: relative; box-shadow: 0 10px 30px var(--shadow-color); border: 1px solid var(--border-color); }
        ...
        .form-input { width: 100%; padding: 0.75rem; background-color: var(--bg-primary); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-size: 1rem; box-sizing: border-box; transition: border-color 0.2s; }
      ...
      <div className="app-container">
        <aside className="sidebar">
            ...
            <div className="sidebar-actions">
              ...
              <button className="sidebar-button" title="Toggle Theme" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}>
          ...
          {currentSong ? (
            ...
            <div className="current-track-info">
              <img src={currentSong.albumArt} alt={currentSong.title} className="footer-album-art" 
                onError={(e) => { e.target.onerror = null; e.target.src='https://placehold.co/56x56/000000/FFFFFF?text=Err'; }}
              />
            ...
            <div style={{gridColumn: '1 / 4', textAlign: 'center', color: 'var(--text-secondary)'}}>
                {isLoading ? 'Loading music...' : 'Select a song to play'}
            </div>
          )}
        </footer>
